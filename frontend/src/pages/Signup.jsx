  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (message.toLowerCase().includes('email already registered')) {
        setError('Email already registered. Sign in or use Forgot password to recover access.')
      } else {
        setError(message)
      }
    } finally {
    ...
    <div className="auth-bg flex items-center justify-center min-h-screen p-4"> 
      <div className="auth-card w-full max-w-sm p-8 fade-in">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-stretch gap-2">
                <select
                  ...
                  className="auth-input"
                  style={{ width: 118, minWidth: 118, flex: '0 0 118px', paddingLeft: 12, paddingRight: 12 }}
                >
        ...
        <p className="text-center text-xs mt-6" style={{ color: 'var(--text-muted)' }}>
          ...
          <Link to="/login" className="font-medium hover:opacity-80" style={{ color: '#b8860b' }}>
            Sign in
          </Link>
        </p>
      </div>
