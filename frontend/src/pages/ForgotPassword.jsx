  ...
  return (
    <div className="flex flex-col items-center mb-8">
      <div
        ...
        style={{ width: 56, height: 56, background: 'linear-gradient(135deg,#14b8a6,#3b82f6)', boxShadow: '0 8px 24px rgba(20,184,166,0.4)' }}
      >
        ...
        <React.Fragment key={s}>
          <div className="flex items-center gap-1.5">
            <div
              ...
              style={{
                background: i <= current ? 'linear-gradient(135deg,#14b8a6,#3b82f6)' : 'rgba(255,255,255,0.06)',
                color: i <= current ? '#fff' : '#4b6282',
                boxShadow: i === current ? '0 0 12px rgba(20,184,166,0.4)' : 'none',
              }}
            >
              {i < current ? '✓' : i + 1}
            </div>
            <span className="text-xs hidden sm:inline" style={{ color: i <= current ? '#2dd4bf' : '#4b6282' }}>{s}</span>
          </div>
          {i < 2 && <div className="w-6 h-px" style={{ background: i < current ? '#14b8a6' : 'rgba(255,255,255,0.1)' }} />}
        </React.Fragment>
      ))}
    </div>
  )
    ...
    <div className="flex gap-2 justify-center" onPaste={handlePaste}>
      {[0,1,2,3,4,5].map(i => (
        <input
          ...
          autoFocus={i === 0}
          style={{
            background: digits[i].trim() ? 'rgba(20,184,166,0.12)' : 'rgba(255,255,255,0.05)',
            border: digits[i].trim() ? '1.5px solid rgba(20,184,166,0.5)' : '1.5px solid rgba(255,255,255,0.08)',
            color: '#2dd4bf',
            height: 52,
          }}
          value={digits[i].trim()}
      ...
      <div className="auth-card w-full max-w-sm p-8 page-in">
        ...
        {step === 0 && (
          <form onSubmit={handleEmail} className="flex flex-col gap-4">
            ...
            <p className="text-center text-xs" style={{ color: 'var(--text-muted)' }}>
              <Link to="/login" className="font-medium hover:opacity-80" style={{ color: '#2dd4bf' }}>Back to login</Link>
            </p>
          </form>
        )}
        ...
        {step === 1 && (
          <form onSubmit={handleOtp} className="flex flex-col gap-4">
            ...
            <button type="button" className="text-xs text-center" style={{ color: '#2dd4bf' }}
        ...
        {step === 3 && (
          <div className="text-center py-4 slide-up">
            <div className="text-5xl mb-4" style={{ filter: 'drop-shadow(0 0 12px rgba(34,197,94,0.5))' }}>✅</div>
